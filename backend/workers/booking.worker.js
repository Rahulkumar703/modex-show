...
const bookingWorker = new Worker("bookingQueue", async job => {
    ...
    return prisma.$transaction(async tx => {
        ...
        const conflicts = await tx.booking.findMany({
            where: {
                seatNo: {in: seatNumbers}, status: "CONFIRMED", bookingRequest: {showId}
            }, select: {seatNo: true}
        });

        if (conflicts.length > 0) {
            await tx.bookingRequest.update({
                where: {id: requestId}, data: {status: "FAILED"}
            });
        ...
        await tx.booking.createMany({
            data: seatNumbers.map(seatNo => ({
                bookingRequestId: requestId, seatNo, status: "CONFIRMED", showId
            }))
        });

        await tx.bookingRequest.update({
            where: {id: requestId}, data: {status: "CONFIRMED"}
        });
